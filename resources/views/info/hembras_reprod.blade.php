...
<div class="row mb-2">
	<div class="col">
		<a href="{{ route('catalogohemrepro.reporte', $finca->id_finca) }}" target="_blank" class="btn btn-default float-right"><i class="fas fa-print"></i></a>
		
		<a href="{{route('ficha', $finca->id_finca)}}" class="btn btn-success float-right mr-2" title="Crear Serie" alt="Crear Serie"><i class="fas fa-plus"></i></a>
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">Series (Hembras) En Capacidad Reproductivas</h3>
                <div class="card-tools">
	                <form method="GET" action="{{route('series_hembras_productivas',$finca->id_finca) }}" role="search">
            ...
            <div class="card-body table-responsive p-0">
                <table class="table table-hover text-nowrap">
                  ...
                  <tbody>
                  	@foreach($serieshembrasrepro as $item)
                    <tr>
                      <td>{{ $item->serie }}</td>
                      <td>{{ $item->codmadre }}</td>
                      <td>{{ $item->codpadre }}</td>
